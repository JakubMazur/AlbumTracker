	...
	private func loadRelease(identifier: UInt) {
		MainRelease.fetch(endpoint: .release(identifier)) { (results) in
			...
			case .success(let mainRelease):
				/*
					For some reason `.onAppear` is called twice while navigation. Found out that may be two reasons behind this. This is connected to "2020-07-14 13:47:47.046687+0200 AlbumTracker[75405:6038286] [UIContextMenuInteraction] Attempting -[UIContextMenuInteraction dismissMenu], when not in an active state. This is a client error most often caused by calling dismiss more than once during a given lifecycle. (<_UIVariableGestureContextMenuInteraction: 0x6000035818f0>)"
